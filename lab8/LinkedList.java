...
//		ll.deleteAtIndex(4);
	...
	public int count() {
		return this.recursiveCount(head, 0);
	}
	
	private int recursiveCount(Node node, int count) {
		if(node == null) {
			return count;
		}
		
		return this.recursiveCount(node.next, count + 1);
	}
	...
	private void recursiveReverse(Node prev, Node current) {
		if(current == null) {
			return;
		}
